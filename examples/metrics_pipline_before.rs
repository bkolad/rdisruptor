//! BEFORE: pipeline whose events carry a shared `Arc<Vec<u8>>`, cloned per
//! publish and per DbWriter event — every clone/drop is an atomic RMW on one
//! shared refcount cache line touched by two threads.
...
fn main() {
    ...
    println!("variant      : pipline BEFORE (shared Arc payload)");
